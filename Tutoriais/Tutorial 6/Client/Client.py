...
if __name__ == "__main__":
	...
	if len(sys.argv)!=4:
		...
		if (not os.path.exists("Files/"+file)):
			...
			TrustedAutKey=open('TrustedAuthority.pub','r').read()  # chave publica da entidade confiavel
			TrustedAutKey=RSA.importKey(TrustedAutKey)
			KeyAndSign=s.recv(450+617)  # recepcao da chave publica do servidor e da assinatura
			ServerPublicKey=RSA.importKey(KeyAndSign[:450])  # chave publica recebida
			sign=(int(KeyAndSign[450:]),)  # assinatura recebida

			new=SHA256.new(ServerPublicKey.exportKey("PEM")).hexdigest()
			if TrustedAutKey.verify(new,sign)==False:  # se a assinatura nao se verificar nao se faz a conneccao
				print("Connection Refused - Public Key is not trusted")
