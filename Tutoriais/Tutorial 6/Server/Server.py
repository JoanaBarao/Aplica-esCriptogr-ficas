...
if __name__ == "__main__":
	...
	if len(sys.argv)!=3:
    	...
    	while True:
			...
			ServerPrivateKey=open('keys/ServerKey.priv','r').read()  # chave privada do servidor
			ServerPrivateKey=RSA.importKey(ServerPrivateKey)
			ServerPublicKey=open('keys/ServerKey.pub','r').read()  # chave publica do servidor
			ServerPublicKey=RSA.importKey(ServerPublicKey).exportKey("PEM") # tam=450
			ServerPublicKeySign=open('keys/ServerKey.pub.sign','r').read() # assinatura da entidade - tam=617

			client.sendall(ServerPublicKey+ServerPublicKeySign)  # envio da chave publica do servidor e da assinatura da entidade
